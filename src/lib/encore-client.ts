...
export namespace Booking {
  export class ServiceClient {
    ...
    public async createBooking(
      params: bookings.CreateBookingParams,
    ): Promise<bookings.BookingResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI(
        "POST",
        `/bookings`,
    ...
    public async getUserBookings(): Promise<bookings.GetUserBookingsResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI("GET", `/bookings/user`);
      return (await resp.json()) as bookings.GetUserBookingsResponse;
    }

    public async listBookings(
      params: bookings.ListBookingsParams,
    ): Promise<bookings.ListBookingsResponse> {
      // Convert our params into the objects we need for the request
      const query = makeRecord<string, string | string[]>({
        apartmentId: String(params.apartmentId),
    ...
    public async updateBookingStatus(
      bookingId: string,
      params: bookings.UpdateBookingParams,
    ): Promise<bookings.BookingResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI(
        "PUT",
...
export namespace guests {
  ...
  export class ServiceClient {
    ...
    public async listGuests(): Promise<ListGuestsResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI("GET", `/guests`);
      return (await resp.json()) as ListGuestsResponse;
    }
  }
}
...
export namespace Property {
  export class ServiceClient {
    ...
    public async getAvailableProperties(
      params: properties.GetAvailablePropertiesParams,
    ): Promise<properties.GetAvailablePropertiesResponse> {
      // Convert our params into the objects we need for the request
      const query = makeRecord<string, string | string[]>({
        dateFrom: params.dateFrom,
        dateTo: params.dateTo,
        guestCount:
          params.guestCount === undefined
            ? undefined
            : String(params.guestCount),
      });
    ...
    ): Promise<properties.GetPropertyResponse> {
      // Convert our params into the objects we need for the request
      const query = makeRecord<string, string | string[]>({
  ...
  constructor(baseURL: string, options: ClientOptions) {
    ...
    if (typeof globalThis === "object" && !("window" in globalThis)) {
      this.headers["User-Agent"] =
        "eci-6kci-Generated-TS-Client (Encore/v1.46.16)";
    }
